...
const fs = window.myremote.fs;//require('fs');
const path = window.myremote.path;//require('path');
const defaultsDeep = require('lodash.defaultsdeep');
const sqlectron = window.myremote.sqlectron;//require('sqlectron-core');

let config;

const cryptoSecret = 'j[F6Y6NoWT}+YG|4c|-<89:ByJ83-9Aj?O8>$Zk/[WFk_~gFbg7<wm+*V|A{xQZ,';

var get=function getConfiguration(cleanCache) {
...
window.myremote.config={get:get};