...
const logger = (0, _logger.default)('db:clients:mysql');
const mysqlErrors = {
...
async function listRoutines(conn) {
  const sql = `
...
function query(conn, queryText) {
  let pid = null;
  let canceling = false;
  const cancelable = (0, _utils.createCancelablePromise)({ ..._errors.default.CANCELED_BY_USER,
...
async function listDatabases(conn, filter) {
  ...
  return data.filter(item => filterDatabase(item, filter, 'Database')).map(row => row.Database);
}
...
async function getRoutineCreateScript(conn, routine, type) {
