...
const logger = (0, _logger.default)('db:clients:sqlite');
const sqliteErrors = {
...
async function truncateAllTables(conn) {
  await runWithConnection(conn, async connection => {
    ...
    `).join(''); // TODO: Check if sqlite_sequence exists then execute:
...
function runWithConnection(conn, run) {
  return new Promise((resolve, reject) => {
    const db = new _sqlite.default.Database(conn.dbConfig.database, async err => {
      if (err) {
        reject(err);
        return;
      }
