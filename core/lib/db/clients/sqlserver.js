...
const logger = (0, _logger.default)('db:clients:sqlserver');
const mmsqlErrors = {
  CANCELED: 'ECANCEL'
};
...
async function disconnect(conn) {
  const connection = await new _mssql.Connection(conn.dbConfig);
  connection.close();
}
    ...
    execute() {
      return runWithConnection(conn, async connection => {
        ...
        try {
          ...
          const commands = identifyCommands(queryText).map(item => item.type); // Executing only non select queries will not return results.
...
async function listTableColumns(conn, database, table) {
  const sql = `
...
async function listTableTriggers(conn, table) {
...
async function listTableIndexes(conn, database, table) {
...
async function getTableReferences(conn, table) {
  const sql = `
...
async function getTableCreateScript(conn, table) {
...
async function getViewCreateScript(conn, view) {
...
async function getRoutineCreateScript(conn, routine) {
  const sql = `
...
function parseRowQueryResult(data, request, command) {
  ...
  return {
    ...
    rows: data,
    fields: Object.keys(data[0] || {}).map(name => ({
      name
    })),
...
async function driverExecuteQuery(conn, queryArgs) {
...
async function runWithConnection(conn, run) {
  const connection = await new _mssql.Connection(conn.dbConfig).connect();
